...
import fr.pandonia.tools.npc.api.events.NPCHideEvent;
import fr.pandonia.tools.npc.api.events.NPCShowEvent;
import fr.pandonia.tools.npc.api.skin.Skin;
...
public abstract class NPCBase implements NPC, NPCPacketHandler {
    ...
    protected final String name = "§8NPC-" + uuid.toString().replace("-", "").substring(0, 10);
    ...
    @Override
    public void updateLines(Player p) {
        this.hologram.update(p, this.hologram.getUpdatePackets(text));
        this.hologram.show(p);
    }
