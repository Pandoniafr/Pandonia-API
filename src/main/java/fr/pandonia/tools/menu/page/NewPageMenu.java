...
public abstract class NewPageMenu extends PandoniaMenu {

    public int page = 0;
    public List<PageItem> pageItems = new ArrayList<>();
    ...
    public void addItem(ItemStack item, PandoniaMenu.ItemClickEvent e){
        pageItems.add(new PageItem(item, e));
    }
    ...
    public void handleMenu(InventoryClickEvent e) {
        ...
        if(e.getClickedInventory().equals(this.inventory)){
            for (PageItem item : pageItems) {
                if(item.isOnPage() && item.getSlot() == e.getSlot()){
            ...
            if(e.getCurrentItem().getType().equals(getNextItem().getType())){
                if(e.getCurrentItem().getItemMeta().getDisplayName() != null && e.getCurrentItem().getItemMeta().getDisplayName().equals(getNextItem().getItemMeta().getDisplayName())
                    ...
                    page++;
                }else if(page != 0 && e.getCurrentItem().getItemMeta().getDisplayName() != null && e.getCurrentItem().getItemMeta().getDisplayName().equals(getPreviousItem().getItemMeta().getDisplayName())){
                    page--;
                }
                setMenuItems();
            }
    ...
    public void setItemOnPage(){
        ...
        List<PageItem> list = getItemList();
        int h = page * getItemNumber();
        for(int i = getStartSlot(); i < getEndSlot(); i+=9){
            for(int j = i; j < i + getItemPerLine(); j++, h++){
                if(h < list.size()){
                    PageItem pageItem = list.get(h);
                    inventory.setItem(j, pageItem.getItem());
                    pageItem.setSlot(j);
                }else{
